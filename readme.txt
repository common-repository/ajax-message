=== Ajax Message ===
Contributors: keksus
License: GPL 3
Donate link: http://keksus.com/donate.html
Tags: contact, form, contact form, feedback, ajax, 
Requires at least: 3.6
Tested up to: 5.6
Requires PHP: 5.3
Stable tag: 4.8.1
License: GPLv3 
License URI: http://www.gnu.org/licenses/gpl-3.0.html

Send message to email with ajax form

== Description ==

This plugin allows you send messages to email from a page, post or from any other location that you specify in template. It can be used as a shortcode or a widget. 
Has a admin settings page where you can change the names of fields, enable or disable captcha, add custom CSS code. 

### Live Demo

* [http://keksus.com/ae.html](http://keksus.com/ae.html)


== Installation ==

Unzip plugin files and upload them under your '/wp-content/plugins/' directory.
Activate plugin at "Plugins" administration page.

### Usage

1. If you want to use Ajax message form on the page or post, add this shortcode inside the text editor:
	[ae_message]
2. If you want to use Ajax message form in the widget, add this shortcode inside the widget "Ajax message", field "Shortcode":
	[ae_message_widget]

== Upgrade Notice ==

Upgrade normally

== Screenshots ==

1. Admin settings page
2. Ajax message form

== Changelog ==

= 1.0 =
* Initial release

== Upgrade Notice ==

= 21.02.2021 =
Full compatibility with WordPress 5.6
Delete Google+ link
